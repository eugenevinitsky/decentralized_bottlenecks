...
class ReplayBuffer(object):
    ...
    @DeveloperAPI
    def add(self, obs_t, action, reward, obs_tp1, done, info, eps_id, unroll_id, agent_index, weight):
        data = (obs_t, action, reward, obs_tp1, done, info, eps_id, unroll_id, agent_index)
        self._num_added += 1
    ...
    def _encode_sample(self, idxes):
        obses_t, actions, rewards, obses_tp1, dones, infos, eps_ids, unroll_ids, agent_indices = \
                [], [], [], [], [], [], [], [], []
        for i in idxes:
            data = self._storage[i]
            obs_t, action, reward, obs_tp1, done, info, eps_id, unroll_id, agent_index = data
            ...
            unroll_ids.append(unroll_id)
            agent_indices.append(agent_index)
            self._hit_count[i] += 1
        # TODO(@evinitsky) you probably can't make an array of this
        return (np.array(obses_t), np.array(actions), np.array(rewards),
                np.array(obses_tp1), np.array(dones), np.array(infos), np.array(eps_ids),
                np.array(unroll_ids), np.array(agent_indices))
...
class PrioritizedReplayBuffer(ReplayBuffer):
    ...
    def add(self, obs_t, action, reward, obs_tp1, done, info, eps_id, unroll_id, agent_index, weight):
        ...
        super(PrioritizedReplayBuffer, self).add(obs_t, action, reward,
                                                 obs_tp1, done, info, eps_id, unroll_id, agent_index, weight)
        if weight is None:
