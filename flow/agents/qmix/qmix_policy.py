...
import ray
from flow.agents.qmix.mixers import VDNMixer, QMixer
from flow.agents.qmix.model import RNNModel, _get_size, FeedForward
...
class QMixTorchPolicy(Policy):
    ...
    def __init__(self, obs_space, action_space, config):
        ...
        self.is_rnn = config["model"]["custom_options"]["is_rnn"]
        if self.is_rnn:
        ...
        self.model = ModelCatalog.get_model_v2(
            ...
            framework="torch",
            name="model",
            default_model=default_model).to(self.device)
