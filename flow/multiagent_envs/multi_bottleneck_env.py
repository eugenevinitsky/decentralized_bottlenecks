...
ADDITIONAL_RL_ENV_PARAMS = {
    ...
    # use qmix
    "qmix": False,
    # if this is true, agents continue to accrue reward even after they have exited
...
class MultiBottleneckEnv(MultiEnv, DesiredVelocityEnv):
    ...
    def get_state(self, rl_actions=None):
        ...
        if self.reward_after_exit:
            ...
            if self.step_counter in self.left_av_time_dict.keys():
                left_rl_ids = self.left_av_time_dict[self.step_counter]
                for rl_id in left_rl_ids:
        ...
        if self.rew_n_crit > 0:
            ...
            if add_params["congest_penalty"]:
                ...
                if num_vehs > 15 * self.scaling:
                    penalty = 10 * (num_vehs - 15 * self.scaling) / (self.env_params.horizon)
